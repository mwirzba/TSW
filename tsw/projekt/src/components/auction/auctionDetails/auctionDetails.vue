<template>
    <div class="auction-details-container" v-if="auction">
        <div class="form-border" v-on:submit.prevent="onSubmit">
            ...
            <p v-if="auction.archived" class="error-message">OFERTA ZAARCHIWIZOWANA</p>
            ...
            <div class="input-group" style="margin-bottom: 10px"
                ...
                <input v-model="newPrice" id="newPrice" :disabled="auction.auctionBuyer === this.$store.state.userData.username || auctionTimeOut  || errorMess || auction.archived">
                <div class="error-message" >
                    <p v-if="!inputValid && submitted">Nieprawidłowa cena</p>
                    <p v-if="errorMess || auctionTimeOut || auction.archived">Aukcja już się zakończyła</p>
                </div>
            </div>
            <button v-on:click="onSubmit"
                    ...
                    :disabled="auction.auctionBuyer === this.$store.state.userData.username || auctionTimeOut  || errorMess || auction.archived">LICYTUJ
            </button>
            <button v-on:click="onSubmit"
                    v-if="this.$store.state.userData.authenticated && auction.buyNow
                    && auction.auctionOwner !== this.$store.state.userData.username"
                    :disabled="auction.auctionBuyer === this.$store.state.userData.username || auction.archived">Kup teraz.
            </button>
            ...
            <p v-if="auction.buyNow && errorMess" class="error-message">Produkt został już zakupiony</p>
        </div>
    </div>
</template>
